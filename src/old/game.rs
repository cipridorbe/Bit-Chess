...
impl Game {
    ...
    pub fn find_best_move(&mut self, depth: u8, stop: &Arc<AtomicBool>) -> Option<Move> {
        search(stop, &mut self.board, depth, &mut self.tt, &mut self.history, &mut self.counter_move)
    }
