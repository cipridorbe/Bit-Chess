use std::{fs, io::{self, Read as _}, marker::PhantomData, path::Path};
...
#[derive(Debug)]
pub struct InMemoryFile(Vec<u8>);

impl ReadAt for InMemoryFile {
...
pub(crate) fn open_table_file<P: AsRef<Path>>(path: P) -> ProbeResult<InMemoryFile> {
...
impl<S: Position + Syzygy> WdlTable<S, InMemoryFile> {
    pub fn open<P: AsRef<Path>>(
        path: P,
        material: &Material,
    ) -> ProbeResult<WdlTable<S, InMemoryFile>> {
...
impl<S: Position + Syzygy> DtzTable<S, InMemoryFile> {
    pub fn open<P: AsRef<Path>>(
        path: P,
        material: &Material,
    ) -> ProbeResult<DtzTable<S, InMemoryFile>> {
