...
pub struct Tablebase<S: Position + Clone + Syzygy> {
    wdl: FxHashMap<Material, (PathBuf, OnceCell<WdlTable<S, InMemoryFile>>)>,
    dtz: FxHashMap<Material, (PathBuf, OnceCell<DtzTable<S, InMemoryFile>>)>,
    max_pieces: usize,
}
...
impl<S: Position + Clone + Syzygy> Tablebase<S> {
    ...
    fn wdl_table(&self, key: &Material) -> SyzygyResult<&WdlTable<S, InMemoryFile>> {
    ...
    fn dtz_table(&self, key: &Material) -> SyzygyResult<&DtzTable<S, InMemoryFile>> {
        if let Some((path, table)) = self
            .dtz
            .get(key)
